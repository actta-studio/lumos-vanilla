...
export default class Slider extends Component {
	constructor({ lenis }) {
		...
		this.tween = this.horizontalLoop(this.elements.get("products"), {
			paused: true,
			repeat: -1,
			reversed: false,
		});
	...
	remtoPixel(item) {
		...
		let itemValue = itemStyle.getPropertyValue("--spacing");
	...
	horizontalLoop(items, config) {
		...
		let tl = gsap.timeline({
			...
			length = items.length,
			startX = items[0].offsetLeft - this.remtoPixel(items[0]),
			times = [],
			widths = [],
			xPercents = [],
			curIndex = 0,
